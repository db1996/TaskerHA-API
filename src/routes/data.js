...
const FDROID_URL =
    "https://f-droid.org/api/v1/packages/com.github.db1996.taskerha";
const GITHUB_URL =
    "https://api.github.com/repos/db1996/TaskerHa/releases/latest";
...
const stmtGetVersion = db.prepare(
    `SELECT version, fetched_at FROM versions WHERE key = ?`,
);
...
async function fetchFdroid() {
    ...
    const suggested = pkgs.find(
        (p) => p.versionCode === data.suggestedVersionCode,
    );
    const version =
        "v" + (suggested?.versionName ?? pkgs[0]?.versionName ?? "");
    if (!version || version === "v")
        throw new Error("F-Droid returned no version");
    return version;
}
